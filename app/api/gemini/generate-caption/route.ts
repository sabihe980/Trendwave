...
const mockCaptions: Record<string, string> = {
  ...
  facebook: `Transform your multi-page posting routine. Try Postrick's Free Forever plan now! 👍`,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!key) {
      const caption = mockCaptions[targetPlatform] || `Grow your brand with Postrick today. No card required!`;
    ...
    // Standard platform key parsing fallback in case of payload read errors before authenticating
    const targetPlatform = "instagram";
    const fallbackCaption = mockCaptions[targetPlatform] || `Grow your brand with Postrick today! (Auto-recovered caption)`;
